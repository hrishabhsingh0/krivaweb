(function ($) {
    "use strict";
    function mykriva_form() {
        $('.showbtn').on('click', function (e) {
            $(this).parents('.mykriva-form-toggle').siblings('.mykriva-form-show').slideToggle();
            e.preventDefault();
        });
    }

    function mykriva_view_mode_sidebar() {
    ...
    function mykriva_view_mode_nosidebar() {
    ...
    function mykriva_video() {
        $('.buttonvideo').simpleLightboxVideo();
    }

    function mykriva_title_tooltip() {
        $(".mykriva-tooltip").mousemove(function (event) {
            var relX = event.pageX - $(this).offset().left - $(this).children('.mykriva-tooltip-inner').outerWidth() / 2;
            var relY = event.pageY - $(this).offset().top - $(this).children('.mykriva-tooltip-inner').outerHeight() - 5;
            $(this).children('.mykriva-tooltip-inner').css({"top": relY, "left": relX});
        });
    }

    function mykriva_header_sticky($elem) {
        var $this = $elem;
        $this.on('mykriva_header_sticky', function () {
            $this.each(function () {
        ...
        }).trigger('mykriva_header_sticky');
        $(window).on('resize', function () {
            $this.trigger('mykriva_header_sticky');
        });
    }

    function mykriva_vertical_menu($elem) {
        /* SHOW ALL ITEM */
        var _countLi = 0,
    ...
    function mykriva_auto_width_vertical_menu() {
    ...
    function mykriva_animation_tabs($elem, _tab_animated) {
    ...
    function mykriva_init_carousel($elem) {
        $elem.not('.slick-initialized').each(function () {
            ...
            _this.on('init', function (event, slick, direction) {
                mykriva_popover_button();
            });
            _this.slick(_config);
        });
    }

    function mykriva_countdown($elem) {
        $elem.on('mykriva_countdown', function () {
            $elem.each(function () {
                var _this = $(this),
                    _text_countdown = '';
        ...
        }).trigger('mykriva_countdown');
    }

    // category product
    function mykriva_category_product($elem) {
        $elem.each(function () {
            var _main = $(this);
    ...
    function mykriva_magnific_popup() {
    ...
    function mykriva_better_equal_elems($elem) {
        $elem.each(function () {
    ...
    function mykriva_product_gallery($elem) {
        $elem.each(function () {
    ...
    function mykriva_popover_button() {
    ...
    $(window).on("load", function () {
        mykriva_video();
        mykriva_form();
        mykriva_view_mode_sidebar();
        mykriva_view_mode_nosidebar();
        if ($('.owl-slick').length) {
            $('.owl-slick').each(function () {
                mykriva_init_carousel($(this));
            });
        }
        if ($('.mykriva-threed-view').length > 0) {
            mykriva_threesixty();
        }
        if ($('.vertical-menu').length > 0) {
            mykriva_auto_width_vertical_menu();
        }
        if ($('.mykriva-countdown').length) {
            mykriva_countdown($('.mykriva-countdown'));
        }
        if ($('.product-gallery').length) {
            mykriva_product_gallery($('.product-gallery'));
        }
        if ($('.block-nav-category').length) {
            mykriva_vertical_menu($('.block-nav-category'));
        }
        if ($('.widget_mykriva_nav_menu').length) {
            mykriva_vertical_menu($('.widget_mykriva_nav_menu'));
        }
        ...
        if ($('.widget_product_categories .product-categories').length) {
            mykriva_category_product($('.widget_product_categories .product-categories'));
        }
        if ($('.header-sticky .header-wrap-stick').length) {
            mykriva_header_sticky($('.header-sticky .header-wrap-stick'));
        }
        if ($('.equal-container.better-height').length) {
            mykriva_better_equal_elems($('.equal-container.better-height'));
        }
        mykriva_popover_button();
        mykriva_magnific_popup();
        mykriva_title_tooltip();
    });
    // Window resize
    $(window).on("resize", function () {
        if ($('.vertical-menu').length > 0) {
